[{1 :nvim-telescope/telescope.nvim
  ...
  :init (fn []
          ...
          (vim.keymap.set :n :<leader>ss ":lua require('telescope.builtin').current_buffer_fuzzy_find()<CR>" {:noremap true}))
  :config (fn []
            (let [telescope (require :telescope)
              ...
              (telescope.setup
               {:defaults
                {:mappings
                 {:i {:<esc> actions.close}   ; Single ESC closes in insert mode
                  :n {:<esc> actions.close    ; Single ESC closes in normal mode too
                      :q actions.close}}
