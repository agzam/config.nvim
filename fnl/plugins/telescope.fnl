 ...
 {:dependencies
  ...
  (fn []
    (let [telescope (require :telescope)
          ...
          {:extensions
           {:undo {}
            :zoxide
            {:prompt_title "Choose dir"
             :mappings
             {:default
              {:after_action
               (fn [selection]
                 (bi.find_files {:cwd selection.path}))}}}}
           :defaults
           {:vimgrep_arguments
            ...
            {:i {:<esc> actions.close
             ...
             :n {:q actions.close}}}}
          extensions [:fzf :undo :file_browser]]
  ...
  (fn []
    (let [ts (require :telescope)
          bi (require :telescope.builtin)
          fb (. ts :extensions :file_browser :file_browser)
          defaults {:noremap true}
          keys [[:n :<M-x> bi.commands {:desc "Commands"}]

                [:n :<leader>pf bi.find_files {:desc "Find in cwd"}]
                ...
                [:n :<leader>fd ts.extensions.zoxide.list {:desc "Commands"}]]]
